...
public class GameScreen extends javax.swing.JFrame {
    ...
    private Timer timer;
    String currentStartTime;    // game start time
    int score;                  // exercise score
    int timeScore;
    int truthScore;

    int[] choosenSettings;      // for the make random questions
    List<int[]> questions;      // questions on the screen
    List<int[]> questionsForFile;   // for adding report .... [0]-> a, [1]->b, [2]->answer, [3]->given answer
    static Object[] rowData;        // settings came from GameChoice.java

    int[] givenAnswers;             // for the comparing with real answers
    int index = 0;                  // for int[] givenAnswers

    int prevTime = 0;               // the time for the each questions
    List<Integer> timeOfQuestions;  // the list consist of the time for the each questions
    int seconds = 0;                // for the timer
    ...
    private void answerBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_answerBtnActionPerformed
        ...
        if (!questions.isEmpty()) {
            ...
            int[] elements = questions.get(0);      // a x b
            ...
            questions.remove(0); // for the new question 
        } else {
            ...
            String tmpTF = timerField.getText();                //convert min:sec 
            String[] timeParts = tmpTF.split(":");

            int min = Integer.parseInt(timeParts[0]);
            int sec = Integer.parseInt(timeParts[1]);
            int gameTime = (min * 60) + sec;

            answerBtn.enableInputMethods(true);

            String[] rowDataString = new String[6];
            for (int i = 0; i < rowData.length; i++) {
                rowDataString[i] = rowData[i].toString();   //object array to string array
            }
            ...
            for (int i = 0; i < givenAnswers.length; i++) {
                if (questionsForFile.get(i)[2] == givenAnswers[i]) {
                    trueAns++;
                    questionsForFile.get(i)[3] = 1;     // was 0 when initialization
            ...
            for (int i = 0; i < questionsForFile.size(); i++) {
                String reportSpecial = questionsForFile.get(i)[0] + "x" + questionsForFile.get(i)[1] + ","
                        + givenAnswers[i] + ","
            ...
            String message = "Time to finish the game: " + gameTime + " sec. Your total score: " + score;
    ...
    private void startButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_startButtonActionPerformed
        ...
        timer = new Timer(1000, (ActionEvent e) -> {
            ...
            String timerText = String.format("%02d:%02d", minute, second);
