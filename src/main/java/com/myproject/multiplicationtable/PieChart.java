...
public class PieChart extends JFrame {
    ...
    private DefaultPieDataset createDataset() {
        ...
        for (Questions setting : settings) {
            questionNumber = setting.settings[4];       // number of questions in an exercise
            for (int i = 0; i < setting.highScores.size(); i++) {
                name = setting.highScores.get(i)[0];    // username of the score table
        ...
        for (Map.Entry<String, Integer> entry : forChart.entrySet()) {
            ...
            System.out.println("key: " + username + ", value: " + number);

            dataset.setValue(username, number);         // for chart
        }
        return dataset;
    }
    ...
    private JFreeChart createChart(DefaultPieDataset dataset) {
        ...
        JFreeChart pieChart = ChartFactory.createPieChart(
                "CHILD - TOTAL SOLVED QUESTIONS",
                dataset,
                false, true, false);
        PiePlot plot = (PiePlot) pieChart.getPlot();
        plot.setLabelGenerator(new StandardPieSectionLabelGenerator("{0} ({1})"));      // numbers in the chart
        return pieChart;
    }
